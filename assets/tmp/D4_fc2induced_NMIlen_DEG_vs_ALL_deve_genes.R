...
induced.genes<-extract_degs(d4wt_vs_d0wt, direction="up", log2FC=2, pval=0.05 )

nrow(induced.genes)
#[1] 3795 (logfc=1) 1915(logfc=2)
...
write.table(venn_list,"./outs/D4_H3K27me3_broad_narrow/venn_genesets_inducedFC2_D4down.tsv",sep="\t", 
col.names = FALSE, 
row.names = TRUE)

induced_D4dko_down<-intersect(induced.genes$gene_id, D4.down$gene_id)

write.table(cbind(induced_D4dko_down),file="./outs/D4_H3K27me3_broad_narrow/inducedFC2_d4ko_down.tsv",
quote=FALSE,
sep="\t",
col.names = FALSE,
row.names = FALSE
...
induced_broad_bed=cbind(induced_broad$chr,
induced_broad$tss_position -1,
induced_broad$tss_position+1,
paste0(induced_broad$id,"_",induced_broad$x),
induced_broad$longitudPicos ,induced_broad$strand)

#tss_info_unique

write.table(induced_broad_bed,
file="./outs/D4_H3K27me3_broad_narrow/inducedFC2_broad.bed",
sep="\t",
col.names = FALSE,
row.names = FALSE,
quote=FALSE
...
m <- 449   # Numero di "broad" negli indotti
n <- 1286 - m  # Numero di non-"broad" nel set "all"
N <- 134    # Numero di "broad" nei down
k <- 64     # Numero di "broad" nell'overlap (indotti e down)
