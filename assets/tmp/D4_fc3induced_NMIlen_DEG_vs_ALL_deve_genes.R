...
induced.genes<-extract_degs(d4wt_vs_d0wt, direction="up", log2FC=3, pval=0.05 )
...
write.table(venn_list,"./outs/D4_H3K27me3_broad_narrow/venn_genesets_inducedFC3_D4down.tsv",sep="\t", 
col.names = FALSE, 
row.names = TRUE)

induced_D4dko_down<-intersect(induced.genes$gene_id, D4.down$gene_id)

write.table(cbind(induced_D4dko_down),file="./outs/D4_H3K27me3_broad_narrow/inducedFC3_d4ko_down.tsv",
quote=FALSE,
sep="\t",
col.names = FALSE,
row.names = FALSE
...
file="./outs/D4_H3K27me3_broad_narrow/inducedFC3_broad.bed",
sep="\t",
col.names = FALSE,
row.names = FALSE,
quote=FALSE
...
m <- 303   # Numero di "broad" negli indotti
n <- 1286 - m  # Numero di non-"broad" nel set "all"
N <- 134    # Numero di "broad" nei down
k <- 49     # Numero di "broad" nell'overlap (indotti e down)
