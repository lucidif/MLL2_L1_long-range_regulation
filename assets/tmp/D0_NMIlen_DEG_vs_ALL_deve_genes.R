...
wk_folder<-paste0(getwd(),"/outs/Lara_multiomic_analysis")
...
load(paste0(wk_folder,"/in/mESC_H3K27me3_levels_linked_toEachTSS.RData"))
d0dkovswt<-read.table(paste0(wk_folder,"/in/build38_DEseq2_RNAseq/D0_DKO_vs_WT.deseq2.results.tsv"),
                      sep="\t",
                      header=TRUE
)
...
proximal_dko_vs_wt<-read.table(paste0(wk_folder,"/outs/great/Double_KO_vs_F_F/nearest/K4me3_proximal_Double_KO_vs_FF/20250123-public-4.0.4-dq6pJp-mm10-all-gene.txt"),
                               sep="\t",
                               header=FALSE
)
...
D0.up<-extract_degs(d0dkovswt, direction="up" )

#D0.nodegs<-extract_degs(d0dkovswt, pval=NULL )

# Definizione no_DEGs: padj non NA, e NON soddisfano i criteri DEG
D0.nodegs<- subset(d0dkovswt, !is.na(padj) & !(padj <= 0.05 & abs(log2FoldChange) >= 1))

# Geni con padj = NA (non testabili per independent filtering di DESeq2)
D0.na_padj <- subset(d0dkovswt, is.na(padj))

nrow(D0.down) + nrow(D0.up) + nrow(D0.nodegs) + nrow(D0.na_padj) == nrow(d0dkovswt)
...
intg_down   <- D0.down$gene_id[D0.down$gene_id %in% tss_h3k27me3Level$gene]
intg_undiff <- D0.nodegs$gene_id[D0.nodegs$gene_id %in% tss_h3k27me3Level$gene]

intg <- rbind(
  cbind(intg_down,   rep("down",   length(intg_down))),
  cbind(intg_undiff, rep("undiff", length(intg_undiff)))
)

intg_preaktss<-merge(intg,tss_h3k27me3Level, by.x=1, by.y=2, all.y=TRUE, sort=TRUE)
intg_preaktss[which(is.na(intg_preaktss$V2)==TRUE),"V2"]<-"other"


length(which(intg_preaktss$longitudPicos==0))
# val0 = 22372
length(which(intg_preaktss$longitudPicos<=broad_thresholds))
# val less broad_thresholds = 25390

length(which(intg_preaktss$longitudPicos>broad_thresholds))
# val less broad_thresholds = 1300

table(intg_preaktss$V2)
...
intg_preaktss2<-intg_preaktss2[which(intg_preaktss2$V2!="other"),]
...
# nodown = geni in proximal_dko_wt_loose ma non in down
# nota: qui uso intg_preaktss6 solo per calcolare i geni, perché a questo punto
# dello script "proximal_dko_wt_loose" non esiste ancora in intg_preaktss2/5
# quindi il set va ricavato da dove è disponibile (vedi nota sotto)

# nodown = geni in proximal_dko_wt_loose ma non in down
set1 <- unique(tss_inside_proximal$gene)
set2 <- unique(intg_preaktss2[which(intg_preaktss2$V2 == "down"), "intg_down"])
nodown_genes <- setdiff(set1, set2)

intg_preaktss_nodown <- intg_preaktss2[which(intg_preaktss2$V2 == "all" &
                                                intg_preaktss2$intg %in% nodown_genes), ]
intg_preaktss_nodown$V2 <- "nodown"

intg_preaktss5<-rbind(intg_preaktss2,
                      intg_preaktss_broad,
                      intg_preaktss_narrow,
                      intg_preaktss_negative,
                      intg_preaktss_nodown
)

colnames(tss_inside_proximal)
colnames(intg_preaktss5)
colnames(intg_preaktss5)[which( colnames(intg_preaktss5)=="intg_down")]<-"intg"
...
write.table(broad_gn_list,
            paste0(wk_folder,"/outs/broad_all_genes.bed"),
            col.names = FALSE,
            row.names = FALSE,
            sep="\t", quote=FALSE)
...
write.table(intg_peaktss4bis,
            paste0(wk_folder,"/outs/broad_narrow_distribution.tsv"),
            col.names = TRUE,
            row.names = FALSE,
            sep="\t", quote=FALSE)
...
nodown_narrow_gn_peaks   <- category_rapresentation(intg_preaktss6, cat.x = "nodown", cat.y = "narrow")
nodown_broad_gn_peaks    <- category_rapresentation(intg_preaktss6, cat.x = "nodown", cat.y = "broad")
nodown_negative_gn_peaks <- category_rapresentation(intg_preaktss6, cat.x = "nodown", cat.y = "negative")

nodown_narrow <- nrow(nodown_narrow_gn_peaks)
nodown_broad  <- nrow(nodown_broad_gn_peaks)
nodown_neg    <- nrow(nodown_negative_gn_peaks)
tnodown <- nodown_narrow + nodown_broad + nodown_neg
...
df<-rbind(
  data.frame(peak.type=c("narrow","broad","negative"),
             regions=c("all","all","all"),
             val=c(all_narrow/(all_narrow+all_broad+all_neg),
                   all_broad/(all_narrow+all_broad+all_neg),
                   all_neg/(all_narrow+all_broad+all_neg))
  ),
  data.frame(peak.type=c("narrow","broad","negative"),
             regions=c("down","down","down"),
             val=c(down_narrow/(down_narrow+down_broad+down_neg),
                   down_broad/(down_narrow+down_broad+down_neg),
                   down_neg/(down_narrow+down_broad+down_neg))
  ),
  # data.frame(peak.type=c("narrow","broad","negative"),
  #            regions=c("undiff","undiff","undiff"),
  #            val=c(undiff_narrow/(undiff_narrow+undiff_broad+undiff_neg),
  #                  undiff_broad/(undiff_narrow+undiff_broad+undiff_neg),
  #                  undiff_neg/(undiff_narrow+undiff_broad+undiff_neg))
  # ),
  data.frame(peak.type=c("narrow","broad","negative"),
             regions=c("proximal_dko_wt_loss","proximal_dko_wt_loss","proximal_dko_wt_loss"),
             val=c(pro_nar/(pro_nar+pro_road+pro_neg),
                   pro_road/(pro_nar+pro_road+pro_neg),
                   pro_neg/(pro_nar+pro_road+pro_neg))
  ),
  data.frame(peak.type=c("narrow","broad","negative"),
           regions=c("nodown","nodown","nodown"),
           val=c(nodown_narrow/(nodown_narrow+nodown_broad+nodown_neg),
                 nodown_broad/(nodown_narrow+nodown_broad+nodown_neg),
                 nodown_neg/(nodown_narrow+nodown_broad+nodown_neg))
  )
)
...
ggsave(paste0(wk_folder,"/outs/H3K27me3_broad_narrow/D0_peak_distribution.png"), width = 10, height = 6, dpi = 300)

write.table(df, file=paste0(wk_folder,"/outs/H3K27me3_broad_narrow/D0_df_barplot.tsv"), sep="\t")
...
# ggsave(paste0(wk_folder,"/outs/H3K27me3_broad_narrow/peak_distribution.png"), width = 10, height = 6, dpi = 300)
