...
sudo nextflow run "${PIPELINE_PATH}" -profile docker \
  ...
  --fasta "/home/lucio/wkdir/projects/MLL2_L1_regulation/in/customReferences/${REF_NAME}/genome.fa" \
  --index "/home/lucio/wkdir/projects/MLL2_L1_regulation/in/customReferences/${REF_NAME}" \
  --outdir "${NF_OUT}" \
...
sudo git/nf-core-microc/bin/v4c_recalc_window_from_pairs_and_bam.sh \
  ...
  --mm10-sizes outs/Lara_multiomic_analysis/in/mm10.sizes \
  --nf-out "${NF_OUT}" \
  --bam-mm10 outs/virtual4C/bw_new_viewp/bw_tracks_chr13_Tcstv3_WT_day4/1500/WT_day4/WT_day4.pooled.viewpoint.filtered.lifted.mm10.bam \
  --outroot outs/virtual4C/bw_new_viewp \
  --smooth 0
